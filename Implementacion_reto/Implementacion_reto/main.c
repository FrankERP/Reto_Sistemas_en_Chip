...
char* duraciones[CANCION_MAX] = {"00:20",
                                    "00:20",
                                    "00:20",
                                    "00:20",
                                    "00:20",
        ...
        if(adcBuffer < 10){
            if((adcFlag & 0XC) != 8){ //Estado actual != Estado anterior
                ...
                if (volumen){ //Si el volumen no es cero. Sin signo
                    volumen -= 1;
                    numLEDs_on = volumen/3;  //VOLUMEN MAX = 30, NUM LEDS = 10
...
ISR(TIMER0_COMP_vect){
    ...
    if (contTimer > 1000){ // 1 s
        ...
        if (play_pause){
            ...
            if((duracionesInt[numCancionActual][0] == minutos) && (duracionesInt[numCancionActual][1] + 1 == segundos)){
                ctrlMP3Flag = (ctrlMP3Flag & 0xF0) | PLAY_NUM_CANCION;
                numCancionActual += 1;
                numCancionDisplay +=1;
                minutos = 0;
